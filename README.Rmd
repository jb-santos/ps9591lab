...
This package contains two sample datasets for use in PS 9591 labs at Western University (Winter 2023).
...
Run the code below in an `R` or `RStudio` session, and it will install the package. You'll need the `{remotes}` package, if you don't already have it (which is why it's included in the code). If you already have `{remotes}`, then just skip the first line. 
...
To load the datasets into your `R` session, use the `data()` command, as shown below.
...
For transparency and replicability, the code used to create the sample datasets is below.
